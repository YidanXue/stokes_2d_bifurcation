% MATLAB code to reproduce Fig. 4
% Yidan Xue, Jul 2023, last update Jun 2024

Alpha = [pi/2 pi/3 pi/2 pi/3 pi/2 pi/2];
Beta = [0 pi/4 pi/4 pi/3 pi/3 pi/2];
Title = {'(a) $\alpha=\pi/2,\ \beta=0$' '(b) $\alpha=\pi/3,\ \beta=\pi/4$' '(c) $\alpha=\pi/2,\ \beta=\pi/4$'...
    '(d) $\alpha=\pi/3,\ \beta=\pi/3$' '(e) $\alpha=\pi/2,\ \beta=\pi/3$' '(f) $\alpha=\pi/2,\ \beta=\pi/2$'};
...
for ii = 1:length(Alpha)
    ...
    [psi,uv,p,omega,f,g] = makefuns(c,Hes,Pol);
     ...
     jj = find(dZw>5e-3); outside = ~inpolygonc(zz,Z(jj));
     nexttile
    ...
    lev = pmin+[.1,.2,.3,.4,.6,.7,.8,.9]*(pmax-pmin);
     contour(x,y,pp,lev,'k',LW,.6)
     lev_cent = pmin+[.5,.5]*(pmax-pmin);
     contour(x,y,pp,lev_cent,'r',LW,.8)
     plot(Z([1:end 1]),'k',LW,.8)
     title(Title(ii),'interpreter','latex')
     hold off, axis equal off
end
set(gcf,'units','inches','position',[0,0,6,6])
exportgraphics(gcf,'./angle_cases.pdf','Resolution',600)
